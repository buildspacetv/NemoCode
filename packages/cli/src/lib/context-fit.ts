...
const IMAGE_REMOVED_PLACEHOLDER = "[nemocode removed an older image to fit the model window]";
const TRIM_MARKER = "\n[nemocode trimmed older context to fit the model window]\n";
...
export function emitContextTrimAlarm(info: ContextTrimTelemetryInfo): void {
  ...
  process.stderr.write(
    `nemocode: ${severity} ${info.trimmedChars} chars of conversation context ` +
