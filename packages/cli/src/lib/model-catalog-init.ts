...
import { applyCatalog, buildCatalog, type NebiusApiModel } from "@nemocode/models";
 ...
 * no hand-maintained list. Results are cached to `~/.nemocode/
...
function cachePath(home?: string): string {
  return path.join(nemocodeHome(home), "model-catalog.json");
}
