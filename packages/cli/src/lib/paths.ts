 ...
 * `resolveKimirelayHome()` copies that previously lived in
 ...
 *
 * `KIMIRELAY_HOME` names the `.kimirelay` directory itself, matching
 ...
 * `$KIMIRELAY_HOME` while `config.json` - which holds the Nebius and Tavily
 * keys - was still read from `~/.kimirelay`.
 *
 ...
 *
 * Every secret kimirelay writes lands under one of these directories: the
