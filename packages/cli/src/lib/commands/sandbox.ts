...
const USAGE = `Usage:
  nemocode sandbox status [--project <id>]   Report your key's Sandboxes permissions
  nemocode sandbox project [<id>|--clear]    Show, store, or clear the Nebius project id
                                              sent on contree (Token Factory) calls
  nemocode sandbox run [--image <tag>] [--timeout <s>] [--keep] [--fetch <path>]... <command...>
                                              Run a shell command in a sandbox. --keep
                                              snapshots the filesystem into a result image;
                                              --fetch (implies --keep) downloads files from
                                              it afterwards
  nemocode sandbox fetch <image-uuid> <path> [--out <file>]
                                              Download one file from a result/checkpoint image
  nemocode sandbox prebake [--image <base>] [--tag <name>]
                                              ...
                                              bootstrap
  nemocode sandbox advisory [--write]        Print (or append) the agent-instructions
...
"nemocode sandbox project <id>" (the id is in the Token Factory console -
there is no API to discover it).

Remote harness sessions (headless, requires a pushed git repo):
  nclaude --sandbox -p "<task>"                Claude Code on Nemotron inside a sandbox
  ncodex --sandbox exec "<task>"               Codex inside a sandbox
 ...
 * Token Factory console - so `nemocode sandbox project <id>` stores it once
...
async function buildClient(apiKeyFlag?: string, projectFlag?: string): Promise<ContreeClient> {
  ...
  if (!apiKey) {
    throw new Error("No Nebius API key found. Run `nemocode configure` or set NEBIUS_API_KEY.");
  }
...
export async function runSandboxCli(args: string[]): Promise<void> {
  ...
  if (verb === "project") {
    ...
    if (value === undefined) {
      ...
      if (!effective) {
        console.log(
          "No Nebius project configured. Store one with `nemocode sandbox project <id>` " +
  ...
  if (verb === "status") {
    ...
    console.log(
      `Provider: ${provider}${opts.provider || process.env.NEMOCODE_SANDBOX_PROVIDER ? "" : " (default)"}. ` +
  ...
  if (verb === "prebake") {
    ...
    console.log(`Use it with: nclaude --sandbox --image tag:${tag} -p "<task>"`);
    return;
  }
  ...
  if (verb === "advisory") {
    ...
    if (!opts.write) {
      ...
      console.log("Append it to your agent instructions with: nemocode sandbox advisory --write");
      return;
    }
...
async function handleRunArtifacts(
  ...
  console.log(
    `Result image: ${status.resultImageUuid} (fetch more later with ` +
      `\`nemocode sandbox fetch ${status.resultImageUuid} <path>\`)`,
  );
...
/**
 * Entry point for `nclaude --sandbox ...` / `ncodex --sandbox ...`: runs the
  ...
  if (passthrough.length === 0) {
    throw new Error(
      `--sandbox runs are headless: pass the harness a task, e.g. ` +
        (harness === "claude"
          ? `nclaude --sandbox -p "fix the failing test"`
          : `ncodex --sandbox exec "fix the failing test"`),
    );
  }
  ...
  if (flags.keep && status.resultImageUuid) {
    console.log(
      `Result image: ${status.resultImageUuid} - download files with ` +
        `\`nemocode sandbox fetch ${status.resultImageUuid} <path>\` (the repo lives at /work).`,
    );
  }
}
