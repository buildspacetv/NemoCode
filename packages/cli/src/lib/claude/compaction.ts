...
const COMPACTION_SIGNATURES = [
  ...
  // them without tying NemoCode to one exact release's wording.
...
export function tuneClaudeCompactionRequest(
  ...
  // value, but NemoCode does not impose a separate compaction-only cap.
...
function boundedCompactionInstruction(
  ...
  return `NemoCode bounded compaction request:
