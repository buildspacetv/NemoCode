...
- Vision models are **fixed, not user-configurable** - curated for the best experience, with automatic failover from primary to fallback:
  - Primary: `nvidia/Cosmos3-Super-Reasoner`.
  - Fallback: `Qwen/Qwen2.5-VL-72B-Instruct`, used if Cosmos errors.
...
- Debug logs (`NEMOCODE_DEBUG=1`): `image blocks detected`, `vision describe start/done` with model, length, and a preview.
...
### Custom `/nemocode-feedback` command
...
- Ship a custom slash command - e.g. `.claude/commands/nemocode-feedback.md` (or a user-level command) - so users can type `/nemocode-feedback <text>` inside a nemocode session.
...
- Capture options to decide between:
  - **Local file** (simplest): append the feedback to a `~/.nemocode/feedback.log` the user can review/share manually. No network, no service.
