...
export function codexAppLaunchMessage(result: CodexAppLaunchResult): string {
  ...
  if (result.wasRunning && result.restartUnsupported) {
    return "ChatGPT App is already open, but nemocode could not restart it. Quit and reopen ChatGPT App when you are ready.";
  }
...
async function shouldRestartCodexApp(reason: CodexAppLaunchReason): Promise<boolean> {
  ...
  const action =
    reason === "restored" ? "reload your restored ChatGPT profile" : "reload the NemoCode profile";
