...
async function daemonStop(): Promise<void> {
  ...
  if (pid === undefined) {
    console.log(`nemocode daemon: not running (no pid file at ${pidPath}).`);
    return;
  }
  try {
    ...
    if (code === "ESRCH") {
      ...
      console.log(`nemocode daemon: not running (stale pid file removed).`);
      return;
    }
    throw err;
  }
  ...
  console.log(`nemocode daemon: stopped (pid ${pid}) on ${daemonUrl(port)}.`);
}
...
async function runInteractiveLauncher(): Promise<void> {
  ...
  if (choice === "chatgpt") {
    ...
    // to the same codex-app flow as `nemocode chatgpt` / `codex-app`.
...
async function main() {
  ...
  // parsing so even `nemocode help` keeps an install current, but it's a
  ...
  // key stored by `nemocode configure`, so the proxy's web search works
  ...
  // Token Factory Sandboxes: `nemocode sandbox <status|run|advisory>`.
  if (command === "sandbox") {
  ...
  // `nclaude --sandbox ...` / `ncodex --sandbox ...`: the wrapper scripts put
